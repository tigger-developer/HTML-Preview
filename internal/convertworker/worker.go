// ABOUTME: Runs a bounded, same-executable native converter protocol.
...
type Request struct {
	...
	SmartOff    bool     `json:"smart_off,omitempty"`
}
...
var errLimit = errors.New("native conversion resource limit exceeded")
...
func Run(input io.Reader, output, diagnostic io.Writer, convert func(Request) ([]byte, error)) int {
	...
	go func() {
		defer func() {
			if recover() != nil {
				result <- conversionResult{err: errors.New("native conversion failed")}
			}
		}()
...
func workerFailure(w io.Writer, err error) int {
	message, code := "native conversion failed", 1
	if errors.Is(err, errLimit) {
		message, code = "native conversion resource limit exceeded", LimitExit
	}
	if err != nil && bytes.Contains([]byte(err.Error()), []byte("memory limit")) {
		message = "native conversion memory limit exceeded"
	}
