...
func TestRT001_14_PrefixInstall(t *testing.T) {
	...
	for _, path := range []string{"LICENSE", "THIRD_PARTY_NOTICES.md", "asap-OFL.txt", "iosevka-custom-OFL.md", "tdewolff-parse-LICENSE.md", "go-yaml-LICENSE"} {
	...
	t.Run("render from installed entry point", func(t *testing.T) {
		...
		for i, name := range []string{"Taḋg  & <notes>.md", "notes.org"} {
			...
			if !contents || !highlighted {
				t.Fatal("W005: installed symlink lost default navigation or highlighting")
			}
		}
	})
}
