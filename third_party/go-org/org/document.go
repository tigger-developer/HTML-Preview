...
type Configuration struct {
	// MaxScanTokenSize bounds an input line; callers supply their admitted input length.
...
func (d *Document) tokenize(input io.Reader) {
	...
	if d.MaxScanTokenSize > 0 {
		scanner.Buffer(make([]byte, min(4096, d.MaxScanTokenSize)), d.MaxScanTokenSize)
	}
	for scanner.Scan() {
